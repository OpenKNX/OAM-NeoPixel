...
void StripConfiguration::configureFromETS()
{
    ...
    for (uint8_t i = 0; i < maxStrips; ++i)
    {
        ...
        if (phys)
        {
            ...
            // Retained only for historic callers: value 0 selects the protocol
            // default and every other selection is an expert override.
            ...
            else
            {
                ...
                if (phys->setCustomTiming(timing.t0hNs, timing.t0lNs, timing.t1hNs, timing.t1lNs, 0))
                    logInfoP("Strip %d: Timing = %u kHz (expert custom T1H=%u ns)", i, freqKhz, timing.t1hNs);
            }
