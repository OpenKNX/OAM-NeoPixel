...
function Generate-EffectMappingCpp {
  ...
  $cppLines += "#include <cstdint>"
  $cppLines += ""
  $cppLines += "/**"
  $cppLines += " * @brief Number of effect types defined (effect IDs are contiguous 0..NEO_EFFECT_TYPE_MAX_ID)"
  $cppLines += " *"
  $cppLines += " * Auto-generated from the EffectPool.cpp registration order so that runtime code"
  $cppLines += " * (e.g. the WebUI effect dropdown) never has to hardcode the highest effect ID."
  $cppLines += " */"
  $cppLines += "constexpr uint8_t NEO_EFFECT_TYPE_COUNT = $($sorted.Count);"
  $cppLines += ""
  $cppLines += "/** @brief Highest valid effect type ID (NEO_EFFECT_TYPE_COUNT - 1). */"
  $cppLines += "constexpr uint8_t NEO_EFFECT_TYPE_MAX_ID = NEO_EFFECT_TYPE_COUNT - 1;"
  $cppLines += ""
  $cppLines += "/**"
